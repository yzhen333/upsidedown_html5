...
function Dialog(content, options)
{
    ...
    var barHtml = !options.showTitle ? '' :
        '<div class="bar"><span class="title">' + options.title + '</span><a class="close">' + options.closeText + '</a><div style="clear:both;"></div></div>';
    ...
    this.show = function()
    {
        ...
        var top=dialog.css({
        	"opacity":getOpacity(options.id),
        	"display":"block"
        }).css("top");
        dialog.css("top","0px").animate({
        	"top":top
        },500, function(){
